...
#SBATCH -c 68
#SBATCH -t 00:30:00
#SBATCH -J GroupByKNL68Flat
#SBATCH -e GroupByKNL68Flat-%j.err
#SBATCH -o GroupByKNL68Flat-%j.out
#SBATCH -L SCRATCH   #Job requires $SCRATCH file system
#SBATCH -C knl,quad,flat   #Use Haswell nodes
...
export SPARK_CONF_DIR="/global/homes/n/nchaimov/groupby-shifter-test/conf-knl-68"
export MASTER=`hostname`

NUM_NODES=16
PRETEND_CORES_PER_NODE=32   
REAL_CORES_PER_NODE=68
export SPARK_WORKER_CORES=68
...
TOTAL_EXECUTOR_CORES=$(( $NUM_NODES * $REAL_CORES_PER_NODE ))
NUM_PARTITIONS=$(( $TOTAL_EXECUTOR_CORES * $PARTITIONS_PER_CORE ))

                                                               
echo ">>>>> Starting servers"
srun -n $NUM_NODES -N $NUM_NODES -c $REAL_CORES_PER_NODE shifter $SPARK_HOME/sbin/start-cluster.sh $MASTER &

sleep 10 # wait for master to start up

echo ">>>>> Running GroupByTest KNL"
echo ">>>>> Max nodes: $NUM_NODES Cores Per Node: $REAL_CORES_PER_NODE"
...
while [ $CUR_NUM_NODES -le $NUM_NODES ] ; do
    TOTAL_EXECUTOR_CORES=$(( $CUR_NUM_NODES * $REAL_CORES_PER_NODE ))
    PRETEND_TOTAL_EXECUTOR_CORES=$(( $CUR_NUM_NODES * $PRETEND_CORES_PER_NODE ))
    NUM_PARTITIONS=$(( $PRETEND_TOTAL_EXECUTOR_CORES * $PARTITIONS_PER_CORE ))
    echo ">>>>> Nodes: $CUR_NUM_NODES Cores: $TOTAL_EXECUTOR_CORES Partitions: $NUM_PARTITIONS"
    shifter $SPARK_HOME/bin/spark-submit --master spark://$MASTER:7077 --total-executor-cores $TOTAL_EXECUTOR_CORES --conf spark.scheduler.minRegisteredResourcesRatio=1.0 --conf spark.executor.cores=$REAL_CORES_PER_NODE $SPARK_HOME/instrumentation-benchmarks/GroupBy/target/scala-2.11/groupbytest_2.11-1.0.jar $NUM_PARTITIONS $KVP_PER_PARTITION $VALUE_BYTES $NUM_PARTITIONS $SEED
