...
#SBATCH -c 32
#SBATCH -t 00:30:00
#SBATCH -J GroupByKNL1W
#SBATCH -e GroupByKNL1W-%j.err
#SBATCH -o GroupByKNL1W-%j.out
...
while [ $CUR_NUM_NODES -le $NUM_NODES ] ; do
    ...
    shifter $SPARK_HOME/bin/spark-submit --master spark://$MASTER:7077 --total-executor-cores 1 --conf spark.scheduler.minRegisteredResourcesRatio=1.0 --conf spark.executor.cores=$CORES_PER_NODE $SPARK_HOME/instrumentation-benchmarks/GroupBy/target/scala-2.11/groupbytest_2.11-1.0.jar $NUM_PARTITIONS $KVP_PER_PARTITION $VALUE_BYTES $NUM_PARTITIONS $SEED
